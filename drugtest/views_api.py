...
from drugtest.serializers import DrugTestModelSerializer
...
class DrugTestListAPIView(ListAPIView):

    serializer_class = DrugTestModelSerializer
    pagination_class = PageNumberPagination

    def get_queryset(self):
        return   DrugTestModel.objects.filter(
            company=self.request.user.default_company_key).order_by('-order_details__date_added')
    ...
    #         serializer = DrugTestModelSerializer(context['page_obj'])
    ...
    #     data = DrugTestModel.objects.filter(
    #         company=user.default_company_key).order_by('-order_details__date_added')
...
class ViewOneDrugTestAPIView(APIView):
    ...
    def get(self, request, **kwargs):
        ...
        try:
            ...
            context = self.get_context_data()
            #serializer = DrugTestModelSerializer
    ...
    def get_context_data(self, **kwargs):
        ...
        cur_model = QuestOrderModel.objects.get(id=id)
...
def searchApiView(request):
    ...
    if request.method == 'POST':
        ...
        if r.status_code == 200:
            ...
            return JsonResponse({"data":"[]","msg": "could not get the values"})
