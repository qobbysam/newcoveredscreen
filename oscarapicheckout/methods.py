...
class SquareTokenSerializer(serializers.Serializer):
    cnon = serializers.CharField()
...
class PaymentMethodSerializer(serializers.Serializer):
    ...
    def validate(self, data):
        ...
        if data["pay_balance"]:
            ...
            raise serializers.ValidationError(
                _("Payment Token from square Must be included")
            )
        return data
...
class SquarePayment(PaymentMethod):
    """
  
    """

    # Translators: Description of payment method in checkout
    name = _("SquarePayment")
    code = "square"
    ...
    def _record_payment(self, request, order, method_key, amount, reference, **kwargs):
        ...
        token = kwargs['token']
        to_use_token = token['cnon']
        #billing_add = kwargs['billing_address']
        idempotency = str(order) + str(date.today())
        total_ = amount
        amount_to_use = float(total_)

        result = facade.authorize(to_use_token,amount_to_use,idempotency)

        res_obj = Result(result, order)
        ...
        if res_obj.isSuccess():
            ...
            return states.Declined(amount, source_id=source.pk)
