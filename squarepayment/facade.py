...
from squarepayment.gateway import Gateway
...
class Facade(object):
    def __init__(self):
        self.squareclient = Gateway()
        #self.mollie.setApiKey(settings.MOLLIE_API_KEY)


    def authorize(self,token,amount, idempotency):
        ...
        body = gateway.generate_body(
                                source_id=token, 
                                idempotency_key=idempotency,
                                amount=amount, 
                                currency=currency)

        result = gateway.create_payment_attempt(body=body)

        return result


    def atomic_payment(self,token,user):
        order = self.get_order(user)

        amount, idempotency = self.get_order_info(order)
        ...
        #res = Result(result, order)

        api_msg = self.api_resolve(result=result, order=order)

        return api_msg

    def api_resolve(self, result, order):
        result_obj = Result(result, order)
        msg_out = {}
        if result_obj.isError():
            msg_out['body'] = result_obj.builderrorMsg()
            msg_out['status'] = status.HTTP_400_BAD_REQUEST
            return msg_out
        if result_obj.isSuccess():
            msg_out['body'] = result_obj.buildsuccessMsg()
            msg_out['status'] = status.HTTP_201_CREATED
...
# '''
# body = {
#     "source_id" : {""} //json post token
#     "idempotency_key" : "u key" // basketid
