...
class ConsortiumAPIView(APIView):
    ...
    def get(self, request):
        ...
        try:
            ...
            context = self.get_loc_data()
            print(context)
            consortium = ConsortiumModelSerializer(context['consortium'])

            response = {}
            response['consortium'] = consortium.data
            response['consortium_ms'] = context['consortium_ms']
            print(response)
            return Response(response, status=status.HTTP_200_OK)

        except Exception as e:
            print(e)
            return Response({'msg': 'BAD Reques'}, status=status.HTTP_400_BAD_REQUEST)



    def get_loc_data(self, **kwargs):
        context =  {}

        try:
            
            company = UserCompanyModel.objects.get( id =  self.request.user.default_company_key)
            context['consortium'] = ConsortiumModel.objects.get(company=company)
            context['consortium_ms'] = json.dumps(context['consortium'].generate_ms())

        except Exception as e:
            print("comoaby does not exist", e)

            context['consortium'] = []

            context['consortium_ms'] = json.dumps( {'active': False})
...
class MyOrdersView(PageTitleMixin,generic.ListView):
    template_name = 'userapplication/orders_index.html'
...
class OrderDetailView(PageTitleMixin, PostActionMixin, generic.DetailView):
    ...
    def get_template_names(self):
        return ['userapplication/order_detail.html']
...
# class DashHomeView(TemplateView):
#     template_name = 'userprofile/dash.html'
